...
| **天数** | **学习主题**                | **资源链接**                                                                                                    | **目标**                                     |
| ------ | ----------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| 1      | Python & PyTorch 基础     | [PyTorch 60min Blitz](https://pytorch.org/tutorials/beginner/deep_learning_60min_blitz.html)                | 掌握 Python 基础语法、PyTorch 张量操作与训练循环           |
| 2      | 手撕神经网络训练                | 教程: [Neural Networks from Scratch](https://www.youtube.com/playlist?list=PLQVvV_e_eKMAN-J94Y-s-A_6SDhY-बोध) | 从零实现一个简单的前馈神经网络，理解反向传播                     |
| 3      | Transformer 宏观理解        | 博客: [The Illustrated Transformer](http://jalammar.github.io/illustrated-transformer/)                       | 掌握 Encoder/Decoder 结构、Multi-Head Attention |
| 4      | Transformer 矩阵计算        | 教程: [Transformer from scratch in Excel](https://www.youtube.com/watch?v=k_P-tprA6-Q)                        | 逐个公式推导 Q/K/V 计算流程                          |
| 5      | 手撕 Multi-Head Attention | 教程: [Let's build GPT: from scratch](https://www.youtube.com/watch?v=kCc8FmEb1nY)                            | 纯 PyTorch 实现 Multi-Head Attention 和 FFN    |
| 6      | 手撕 Transformer 关键模块     |                                                                                                             | 组合已实现模块，完成一个完整的 Transformer Block          |
| 7      | MOE 架构与模型家族             | 论文: [Outrageously Large Neural Networks](https://arxiv.org/abs/1701.06538)                                  | 理解 MOE 架构，并梳理 Bert、T5、GPT 架构的差异            |
...
| **天数** | **学习主题**                | **资源链接**                                                                              | **目标**                                               |
| ------ | ----------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| 8      | Agent 核心概念              | 博客: [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) | 建立 Agent 的宏观认知，理解其与 LLM 的区别                          |
| 9-10   | ReAct 论文精读与复现       | 论文: [ReAct](https://arxiv.org/abs/2210.03629)<br>代码: [LangChain ReAct Agent](https://python.langchain.com/v0.1/docs/modules/agents/agent_types/react/) | 深度理解 "Thought, Action, Observation" 循环，并用框架实现 |
| 11-12  | ReAct 算法复现与思考           | 博客: [深入理解 ReAct](https://www.promptingguide.ai/techniques/react) | 总结 ReAct 的优缺点，思考其在复杂任务中的局限性 |
...
| **天数** | **学习主题** | **资源链接** | **目标** |
|---------|-------------|-------------|---------|
| 15 | Reflexion 论文精读 | 论文: [Reflexion](https://arxiv.org/abs/2303.11366)<br>解读: [Reflexion 论文解读](https://www.interconnects.ai/p/reflexion) | 掌握其"Actor -> Evaluator -> Self-Reflection"的算法流程 |
| 16 | Reflexion 算法分析 | 伪代码: [Reflexion 官方伪代码](https://github.com/noahshinn/reflexion/blob/main/ALGORITHM.md) | 分析反思机制如何帮助 Agent 从失败中学习，并尝试用伪代码实现 |
| 17 | Tree of Thoughts 论文精读 | 论文: [Tree of Thoughts](https://arxiv.org/abs/2305.10601)<br>代码: [ToT 开源代码实现](https://github.com/kyegomez/tree-of-thoughts) | 理解如何将 LLM 作为搜索算法的启发式函数 |
| 18 | ToT 算法实战 | 任务: [24点游戏](https://en.wikipedia.org/wiki/24_Game) | 实现一个简化的 ToT 搜索策略来解决 24点游戏 |
| 19 | Multi-Agent 协作模式 | 论文: [MetaGPT](https://arxiv.org/abs/2308.00352) | 学习 MetaGPT 中角色定义 (SOPs) 和协作模式 |
| 20 | AutoGen 框架实战 | 文档: [AutoGen 官方教程](https://microsoft.github.io/autogen/docs/getting-started/) | 使用 AutoGen 搭建一个简单的 Coder 和 Critic Agent |
...
| **天数** | **学习主题** | **资源链接** | **目标** |
|---------|-------------|-------------|---------|
| 22 | 检索算法基础 (BM25) | 教程: [BM25 from scratch](https://www.pinecone.io/learn/series/bm25/bm25-pragmatic-guide/) | 理解 TF-IDF 和 BM25 的原理，并手动实现 |
| 23 | DPR 与密集检索 | 论文: [DPR](https://arxiv.org/abs/2004.04906)<br>教程: [Sentence Transformers](https://www.sbert.net/) | 掌握双编码器架构，并使用 Sentence Transformers 训练一个模型 |
| 24 | Reranker 与混合检索 | 教程: [LlamaIndex Reranking](https://docs.llamaindex.ai/en/stable/examples/node_postprocessor/MetadataFilter.html) | 理解 Reranker 的作用，并实现一个 BM25 + Embedding 的混合检索流程 |
| 25 | GraphRAG 技术解读 | 报告: [Microsoft GraphRAG](https://www.microsoft.com/en-us/research/project/graphrag/)<br>博客: [GraphRAG 详解](https://aka.ms/graphrag-blog) | 理解其基于图的社群检测、摘要和问答流程 |
| 26 | RAG 评估体系 | 文档: [RAGAs 评估框架](https://docs.ragas.io/en/latest/index.html) | 学习 Faithfulness, Answer Relevancy 等 RAG 评估指标，并用 RAGAs 进行评估 |
| 27 | Self-RAG 论文精读 | 论文: [Self-RAG](https://arxiv.org/abs/2310.11511) | 学习如何通过 "reflection tokens" 让 LLM 自主决定何时检索、检索什么内容 |
| 28 | Agentic RAG 算法设计 | | 思考如何设计一个能进行多步推理的 Agentic RAG 策略，并绘制流程图 |
...
| **天数** | **学习主题** | **资源链接** | **目标** |
|---------|-------------|-------------|---------|
| 29 | Agent Memory 概述 | 博客: [LLM Powered Agents - Memory](https://lilianweng.github.io/posts/2023-06-23-agent/#memory) | 梳理 Agent 记忆的分类和挑战 |
| 30 | Generative Agents 论文精读 | 论文: [Generative Agents](https://arxiv.org/abs/2304.03442) | 学习其对记忆进行评分 (Recency, Importance, Relevance) 和检索的机制 |
| 31 | MemGPT 论文精读 | 论文: [MemGPT](https://arxiv.org/abs/2310.08560)<br>代码: [MemGPT 开源库](https://github.com/cpacker/MemGPT) | 学习其分层记忆和函数调用管理虚拟上下文的方法 |
| 32 | MemGPT 实战 | | 运行 MemGPT 官方示例，理解其工作流程 |
| 33 | 上下文压缩技术 | 教程: [LlamaIndex Context Stuffing](https://docs.llamaindex.ai/en/stable/module_guides/querying/response_synthesizers/root.html) | 学习并实现不同的上下文填充和压缩策略 |
| 34 | 上下文选择与过滤 | 教程: [LlamaIndex Node Postprocessors](https://docs.llamaindex.ai/en/stable/module_guides/querying/node_postprocessors/node_postprocessors.html) | 实现一个自定义的后处理器来优化上下文选择 |
...
>   - 论文: https://arxiv.org/abs/2406.19215
>
> **手撕与学习任务:**
> - [ ] 推导 DPO 的损失函数
...
| **天数** | **学习主题**              | **资源链接**                                                                                          | **目标**                               |
| ------ | --------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------ |
| 36     | RL 基础入门               | 教程: [Hugging Face Deep RL Course](https://huggingface.co/learn/deep-rl-course/unit0/introduction) | 掌握 MDP, Policy, Value Function 等核心概念 |
| 37     | Policy Gradient & PPO | 博客: [Understanding PPO](https://spinningup.openai.com/en/latest/algorithms/ppo.html)              | 理解 PPO 的目标函数和裁剪机制                    |
| 38     | DPO 论文精读与推导           | 论文: [DPO](https://arxiv.org/abs/2305.18290)<br>博客: [DPO 详解](https://huggingface.co/blog/dpo-trl)  | 掌握 DPO 如何从偏好数据中隐式学习奖励并优化策略，并推导其损失函数  |
| 39     | DPO 实战                | 教程: [Hugging Face TRL DPO](https://huggingface.co/docs/trl/main/en/dpo_trainer)                   | 使用 `TRL` 库完成一次 DPO 训练                |
| 40     | GRPO 理论解读             | 论文: [GRPO](https://arxiv.org/abs/2406.19215)                                                      | 理解 GRPO 如何将 DPO 扩展到组级别的偏好            |
| 41     | RL for Tool Learning  | 论文: [Toolformer](https://arxiv.org/abs/2302.04761)                                                | 学习如何用 RL 思想让模型学会使用工具                 |
| 42     | 奖励模型设计                | 教程: [TRL Reward Modeling](https://huggingface.co/docs/trl/main/en/reward_trainer)                 | 学习如何为 Agent 任务设计奖励函数/训练奖励模型          |
...
| **天数** | **学习主题**                  | **资源链接**                                                                                                         | **目标**                             |
| ------ | ------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| 61     | 论文撰写 (Intro & Conclusion) | 模板: [Overleaf ACL Template](https://www.overleaf.com/latex/templates/acl-2023-proceedings-template/qjdgcrdwvxpf) | 完成引言、结论和摘要部分的初稿，并进行全文校对            |
| 62     | 代码开源与博客撰写                 | 指南: [如何写好 README](https://www.makeareadme.com/)                                                                  | 整理代码，撰写 README，并写一篇技术博客解读你的项目      |
| 63     | 简历项目经历打磨                  |                                                                                                                  | 按照 STAR-算法版 模板，将你的项目经历写入简历         |
| 64     | 准备项目介绍逐字稿                 |                                                                                                                  | 准备一个 3-5 分钟的项目介绍，覆盖 S/T/A/R 各个环节   |
| 65     | 模拟项目深挖                    |                                                                                                                  | 针对 "为什么不用XX方法"、"算法的局限性" 等问题准备回答    |
| 66     | 算法理论题复习                   | 题库: [AI Interview Questions](https://github.com/Complete-AI-Recruiting/AI-Interview-Questions)                   | 复习 Transformer, RL, RAG 等核心理论高频面试题 |
| 67     | 模拟面试与总结                   |                                                                                                                  | 进行 1v1 模拟面试，复盘并改进                  |
